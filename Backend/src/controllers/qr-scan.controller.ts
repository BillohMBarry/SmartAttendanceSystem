 ...
 */
export const handleCheckInScan = (req: Request, res: Response) => {
    try {
        ...
        // Token is valid - redirect to frontend with the token
        const frontendUrl = config.frontendUrl;
        const redirectUrl = `${frontendUrl}/check-in?qr_token=${encodeURIComponent(qr_token)}`;

        logger.info({ officeId: decoded.officeId }, 'Valid QR token scanned, redirecting to frontend');
        ...
        return res.send(`
            ...
            <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title>Redirecting to Check-In</title>
            ...
            <body>
                <div class="container">
                    ...
                    <p>Redirecting you to check-in...</p>
        ...
        logger.error({ error }, 'Error handling check-in QR scan');
...
/**
 * Handle QR code scan for check-out
 * This is a public GET endpoint that users hit when they scan a QR code
 * Redirects to the frontend with the token
 */
export const handleCheckOutScan = (req: Request, res: Response) => {
    try {
        const { qr_token } = req.query;
        ...
        // Token is valid - redirect to frontend with the token
        const frontendUrl = config.frontendUrl;
        const redirectUrl = `${frontendUrl}/check-out?qr_token=${encodeURIComponent(qr_token)}`;

        logger.info({ officeId: decoded.officeId }, 'Valid QR token scanned for check-out, redirecting to frontend');
        ...
        return res.send(`
            ...
            <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title>Redirecting to Check-Out</title>
                <meta http-equiv="refresh" content="0;url=${redirectUrl}">
                <style>
                    body { 
                        font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
            ...
            <body>
                <div class="container">
                    <h1>✅ Valid QR Code</h1>
                    <div class="spinner"></div>
                    <p>Redirecting you to check-out...</p>
                    <p><a href="${redirectUrl}" style="color: #667eea;">Click here if you are not redirected automatically</a></p>
                </div>
            </body>
            </html>
        ...
        logger.error({ error }, 'Error handling check-out QR scan');
        return res.status(500).send(`
            <!DOCTYPE html>
            <html>
            <head>
